...
- **Cloudflare API token expiry / scope** (seeded): CI passes but the Deploy step fails with `Authentication error [code: 10000]` when `CLOUDFLARE_API_TOKEN` is expired or lacks scope. This is a static SPA — the token only needs **Cloudflare Pages: Edit** (no D1). **Fix (manual, owner):** regenerate the token (Cloudflare dashboard → My Profile → API Tokens) and update the GitHub secret.
