...
- **Multiplayer deploy needs three things beyond the app** (Phase 4, #23): (1) `CLOUDFLARE_API_TOKEN` must add **Workers Scripts: Edit** scope or the "Deploy rooms Worker" step 403s; (2) the two GitHub Actions **variables** `VITE_ROOMS_HTTP_BASE`/`VITE_ROOMS_WS_BASE` must be set to the deployed Worker origin (`https://`/`wss://`), else the built frontend falls back to same-origin `/api` and multiplayer silently can't connect; (3) the Worker deploy step runs **before** the Pages deploy so the new frontend has a live backend. First-time bootstrap is chicken-and-egg: deploy the Worker once (`npm run rooms:deploy`), read its `workers.dev` URL, set the two vars, then let the pipeline run. The rooms Worker deploy is main-only (not on PRs), so PR previews use the prod rooms backend.
